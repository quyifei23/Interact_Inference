...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 418.627 |
| Interaction → graph entry, correct application rows | 1 | 418.627 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 678.561 |
| Interaction → main entry observed | 1 | 423.427 |
| Host submission | 1 | 40.856 |
| IPC request → owner receipt | 1 | 0.339 |
| IPC round trip (includes owner control when present) | 1 | 527.389 |
| Owner receipt → preparation begin | 1 | 0.478 |
| Common owner preparation | 1 | 46.098 |
| Preparation end → RM call begin | 1 | 1.551 |
| Preparation end → action end (no-op or active) | 1 | 479.808 |
| Action end → IPC acknowledgment | 1 | 0.666 |
| RM syscall wall time (including failures) | 1 | 478.226 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 5.838/145.735. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 5.908/9.728. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.49/147.626. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.918/15.706. Minimum RTT is not a full-run hard error bound.
