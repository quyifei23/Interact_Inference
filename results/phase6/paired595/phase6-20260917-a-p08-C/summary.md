...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 723.341 |
| Interaction → graph entry, correct application rows | 1 | 723.341 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 982.310 |
| Interaction → main entry observed | 1 | 728.229 |
| Host submission | 1 | 39.082 |
| IPC request → owner receipt | 1 | 0.282 |
| IPC round trip (includes owner control when present) | 1 | 40.579 |
| Owner receipt → preparation begin | 1 | 0.312 |
| Common owner preparation | 1 | 39.486 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.035 |
| Action end → IPC acknowledgment | 1 | 0.464 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 7.531/146.814. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 6.676/8.643. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.3/150.069. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.898/18.078. Minimum RTT is not a full-run hard error bound.
