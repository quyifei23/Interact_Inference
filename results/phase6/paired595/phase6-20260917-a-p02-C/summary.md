...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 280.412 |
| Interaction → graph entry, correct application rows | 1 | 280.412 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 537.595 |
| Interaction → main entry observed | 1 | 285.125 |
| Host submission | 1 | 39.952 |
| IPC request → owner receipt | 1 | 0.282 |
| IPC round trip (includes owner control when present) | 1 | 37.784 |
| Owner receipt → preparation begin | 1 | 0.337 |
| Common owner preparation | 1 | 36.593 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.040 |
| Action end → IPC acknowledgment | 1 | 0.532 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 7.437/148.37. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 6.684/10.797. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.403/147.839. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.7/16.583. Minimum RTT is not a full-run hard error bound.
