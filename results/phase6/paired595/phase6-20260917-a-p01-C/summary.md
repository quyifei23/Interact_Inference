...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 2037.684 |
| Interaction → graph entry, correct application rows | 1 | 2037.684 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 2289.445 |
| Interaction → main entry observed | 1 | 2042.188 |
| Host submission | 1 | 23.118 |
| IPC request → owner receipt | 1 | 0.115 |
| IPC round trip (includes owner control when present) | 1 | 34.502 |
| Owner receipt → preparation begin | 1 | 0.219 |
| Common owner preparation | 1 | 33.481 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.035 |
| Action end → IPC acknowledgment | 1 | 0.652 |
| RM syscall wall time (including failures) | 0 | N/A |
| GPU graph marker interval | 1 | 251.904 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 6.742/151.242. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 7.101/8.7. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 6.222/152.905. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 5.97/12.188. Minimum RTT is not a full-run hard error bound.
