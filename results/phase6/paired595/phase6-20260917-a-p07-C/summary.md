...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 433.622 |
| Interaction → graph entry, correct application rows | 1 | 433.622 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 690.868 |
| Interaction → main entry observed | 1 | 438.288 |
| Host submission | 1 | 41.334 |
| IPC request → owner receipt | 1 | 0.288 |
| IPC round trip (includes owner control when present) | 1 | 39.016 |
| Owner receipt → preparation begin | 1 | 0.258 |
| Common owner preparation | 1 | 37.778 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.036 |
| Action end → IPC acknowledgment | 1 | 0.656 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 7.056/146.27. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 7.323/11.535. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 6.416/148.222. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 7.245/7.597. Minimum RTT is not a full-run hard error bound.
