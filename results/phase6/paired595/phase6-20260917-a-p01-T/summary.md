...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 403.284 |
| Interaction → graph entry, correct application rows | 1 | 403.284 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 675.729 |
| Interaction → main entry observed | 1 | 408.073 |
| Host submission | 1 | 24.699 |
| IPC request → owner receipt | 1 | 0.141 |
| IPC round trip (includes owner control when present) | 1 | 505.589 |
| Owner receipt → preparation begin | 1 | 0.291 |
| Common owner preparation | 1 | 31.640 |
| Preparation end → RM call begin | 1 | 0.930 |
| Preparation end → action end (no-op or active) | 1 | 473.004 |
| Action end → IPC acknowledgment | 1 | 0.513 |
| RM syscall wall time (including failures) | 1 | 472.036 |
| GPU graph marker interval | 1 | 272.384 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 6.051/165.252. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 7.459/12.689. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 6.384/167.606. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.677/12.951. Minimum RTT is not a full-run hard error bound.
