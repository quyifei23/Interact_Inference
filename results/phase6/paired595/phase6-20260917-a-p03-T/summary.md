...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 400.796 |
| Interaction → graph entry, correct application rows | 1 | 400.796 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 658.134 |
| Interaction → main entry observed | 1 | 405.401 |
| Host submission | 1 | 40.637 |
| IPC request → owner receipt | 1 | 0.367 |
| IPC round trip (includes owner control when present) | 1 | 509.409 |
| Owner receipt → preparation begin | 1 | 0.334 |
| Common owner preparation | 1 | 36.312 |
| Preparation end → RM call begin | 1 | 1.124 |
| Preparation end → action end (no-op or active) | 1 | 471.732 |
| Action end → IPC acknowledgment | 1 | 0.664 |
| RM syscall wall time (including failures) | 1 | 470.471 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 6.108/148.502. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 6.551/8.644. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.06/149.029. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.498/10.804. Minimum RTT is not a full-run hard error bound.
