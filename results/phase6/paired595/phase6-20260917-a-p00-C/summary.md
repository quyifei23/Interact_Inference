...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 1132.679 |
| Interaction → graph entry, correct application rows | 1 | 1132.679 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 1387.731 |
| Interaction → main entry observed | 1 | 1137.289 |
| Host submission | 1 | 41.314 |
| IPC request → owner receipt | 1 | 0.163 |
| IPC round trip (includes owner control when present) | 1 | 48.761 |
| Owner receipt → preparation begin | 1 | 0.470 |
| Common owner preparation | 1 | 47.411 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.045 |
| Action end → IPC acknowledgment | 1 | 0.672 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 6.981/148.149. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 7.374/23.714. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.577/148.747. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.912/23.102. Minimum RTT is not a full-run hard error bound.
