...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 406.940 |
| Interaction → graph entry, correct application rows | 1 | 406.940 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 667.017 |
| Interaction → main entry observed | 1 | 411.816 |
| Host submission | 1 | 39.915 |
| IPC request → owner receipt | 1 | 0.342 |
| IPC round trip (includes owner control when present) | 1 | 520.434 |
| Owner receipt → preparation begin | 1 | 0.484 |
| Common owner preparation | 1 | 37.954 |
| Preparation end → RM call begin | 1 | 1.280 |
| Preparation end → action end (no-op or active) | 1 | 480.901 |
| Action end → IPC acknowledgment | 1 | 0.753 |
| RM syscall wall time (including failures) | 1 | 479.547 |
| GPU graph marker interval | 1 | 260.096 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 7.423/149.037. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 6.441/11.863. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 6.881/148.699. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.948/11.557. Minimum RTT is not a full-run hard error bound.
