...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 1918.537 |
| Interaction → graph entry, correct application rows | 1 | 1918.537 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 2174.624 |
| Interaction → main entry observed | 1 | 1923.173 |
| Host submission | 1 | 28.123 |
| IPC request → owner receipt | 1 | 0.183 |
| IPC round trip (includes owner control when present) | 1 | 45.251 |
| Owner receipt → preparation begin | 1 | 0.255 |
| Common owner preparation | 1 | 44.400 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.034 |
| Action end → IPC acknowledgment | 1 | 0.379 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 6.582/147.875. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 7.485/9.504. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.049/150.876. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.793/12.89. Minimum RTT is not a full-run hard error bound.
