...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 409.687 |
| Interaction → graph entry, correct application rows | 1 | 409.687 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 666.499 |
| Interaction → main entry observed | 1 | 414.382 |
| Host submission | 1 | 39.479 |
| IPC request → owner receipt | 1 | 0.330 |
| IPC round trip (includes owner control when present) | 1 | 499.766 |
| Owner receipt → preparation begin | 1 | 0.469 |
| Common owner preparation | 1 | 36.941 |
| Preparation end → RM call begin | 1 | 1.652 |
| Preparation end → action end (no-op or active) | 1 | 461.486 |
| Action end → IPC acknowledgment | 1 | 0.540 |
| RM syscall wall time (including failures) | 1 | 459.804 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 7.297/148.13. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 6.686/12.49. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 6.646/146.779. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.932/11.82. Minimum RTT is not a full-run hard error bound.
