...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 422.123 |
| Interaction → graph entry, correct application rows | 1 | 422.123 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 688.843 |
| Interaction → main entry observed | 1 | 427.150 |
| Host submission | 1 | 38.959 |
| IPC request → owner receipt | 1 | 0.280 |
| IPC round trip (includes owner control when present) | 1 | 530.908 |
| Owner receipt → preparation begin | 1 | 0.402 |
| Common owner preparation | 1 | 47.950 |
| Preparation end → RM call begin | 1 | 1.853 |
| Preparation end → action end (no-op or active) | 1 | 481.613 |
| Action end → IPC acknowledgment | 1 | 0.663 |
| RM syscall wall time (including failures) | 1 | 479.718 |
| GPU graph marker interval | 1 | 266.240 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 6.648/150.56. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 8.993/14.09. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.318/153.347. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.842/7.977. Minimum RTT is not a full-run hard error bound.
