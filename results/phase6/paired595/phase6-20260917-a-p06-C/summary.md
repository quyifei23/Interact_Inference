...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 1255.725 |
| Interaction → graph entry, correct application rows | 1 | 1255.725 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 1512.696 |
| Interaction → main entry observed | 1 | 1260.483 |
| Host submission | 1 | 40.944 |
| IPC request → owner receipt | 1 | 0.355 |
| IPC round trip (includes owner control when present) | 1 | 36.689 |
| Owner receipt → preparation begin | 1 | 0.589 |
| Common owner preparation | 1 | 35.145 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.037 |
| Action end → IPC acknowledgment | 1 | 0.563 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 6.516/149.059. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 7.334/7.746. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 6.426/149.096. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 7.023/14.263. Minimum RTT is not a full-run hard error bound.
