...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 1156.234 |
| Interaction → graph entry, correct application rows | 1 | 1156.234 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 1413.622 |
| Interaction → main entry observed | 1 | 1160.872 |
| Host submission | 1 | 41.064 |
| IPC request → owner receipt | 1 | 0.252 |
| IPC round trip (includes owner control when present) | 1 | 46.669 |
| Owner receipt → preparation begin | 1 | 0.370 |
| Common owner preparation | 1 | 45.559 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.034 |
| Action end → IPC acknowledgment | 1 | 0.454 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 6.712/147.166. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 6.74/15.261. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.134/149.962. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 6.397/13.478. Minimum RTT is not a full-run hard error bound.
