...
|---|---:|---:|
| Interaction → graph entry observed, all application-valid | 1 | 413.596 |
| Interaction → graph entry, correct application rows | 1 | 413.596 |
| Interaction → graph entry, timing-eligible subset | 0 | N/A |
| Interaction → graph done observed, all application-valid | 1 | 671.240 |
| Interaction → main entry observed | 1 | 418.287 |
| Host submission | 1 | 39.889 |
| IPC request → owner receipt | 1 | 0.281 |
| IPC round trip (includes owner control when present) | 1 | 38.589 |
| Owner receipt → preparation begin | 1 | 0.255 |
| Common owner preparation | 1 | 37.072 |
| Preparation end → RM call begin | 0 | N/A |
| Preparation end → action end (no-op or active) | 1 | 0.035 |
| Action end → IPC acknowledgment | 1 | 0.946 |
...
bg_calibration_after.csv: ping samples=1000, min/max RTT μs 7.174/146.857. Minimum RTT is not a full-run hard error bound.

bg_calibration_before.csv: ping samples=1000, min/max RTT μs 7.173/9.405. Minimum RTT is not a full-run hard error bound.

int_calibration_after.csv: ping samples=1000, min/max RTT μs 7.066/147.923. Minimum RTT is not a full-run hard error bound.

int_calibration_before.csv: ping samples=1000, min/max RTT μs 7.178/7.809. Minimum RTT is not a full-run hard error bound.
