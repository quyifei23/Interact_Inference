...
inline Options parse(int argc,char** argv){
    ...
    for(int i=1;i<argc;++i){std::string key=argv[i];
        ...
        if(key=="--graph")o.graph=true;
            ...
            std::cout<<"int_worker --probe-cuda | --probe-rm-observe | --probe-rm-identity | --probe-rm-readonly | --probe-rm-group-info [--run-dir DIR]\n"
                     ...
                     <<"int_worker --run-dir DIR --mode int-only|none|timeslice|preempt-wait|group-bound-none|group-preempt-wait|preempt-async|realtime-only|realtime-restart|realtime|disable|disable-split\n"
            ...
            if(key=="--run-dir")o.run_dir=value;else if(key=="--shared")o.shared_path=value;else if(key=="--mode")o.mode=value;
            else if(key=="--batch-id")o.batch_id=value;else if(key=="--pair-id")o.pair_id=value;
            else if(key=="--pair-order")o.pair_order=value;else if(key=="--condition")o.condition=value;else if(key=="--run-id")o.run_id=value;
                ...
                else if(key=="--trigger-delay-us"){if(v<1000||v>5000)throw std::invalid_argument("trigger delay must be 1000..5000 us");o.trigger_delay_us=v;}
    ...
    for(const auto& value:{o.batch_id,o.pair_id,o.pair_order,o.condition,o.run_id})
        if(value.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_.")!=std::string::npos)
            throw std::invalid_argument("Invalid experiment metadata: use letters/digits/-_.");
    if(!o.condition.empty()&&o.condition!=(p.name=="group-bound-none"?"control":p.name=="group-preempt-wait"?"treatment":""))
